...
const PLUGIN_TEMPLATE_NAME = 'console-plugin-capsule';
...
const installHelmChart = (path: string) => {
  ...
  cy.exec(
    `cd ../../console-plugin-capsule && ${path} upgrade -i ${PLUGIN_TEMPLATE_NAME} charts/openshift-console-plugin -n ${PLUGIN_TEMPLATE_NAME} --create-namespace --set plugin.image=${PLUGIN_TEMPLATE_PULL_SPEC}`,
    {
...
const deleteHelmChart = (path: string) => {
  cy.exec(
    `cd ../../console-plugin-capsule && ${path} uninstall ${PLUGIN_TEMPLATE_NAME} -n ${PLUGIN_TEMPLATE_NAME} && oc delete namespaces ${PLUGIN_TEMPLATE_NAME}`,
    {
      failOnNonZeroExit: false,
    },
  ).then((result) => {
...
describe('Console plugin template test', () => {
  before(() => {
    ...
    if (!isLocalDevEnvironment) {
      ...
      cy.exec('cd ../../console-plugin-capsule && ./install_helm.sh', {
        failOnNonZeroExit: false,
      }).then((result) => {
